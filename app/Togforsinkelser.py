...
CSV_PATH = os.path.join(
    project_root,
    "reiser_oslo_sandvika",
    "Alle_reiser_Oslo_Sandvika.csv")

if not os.path.exists(CSV_PATH):
    # Fallback for ulike kjøremiljøer (f.eks. hvis man kjører fra roten uten
    # absolutt __file__)
...
def load_data(path: str, mtime) -> pd.DataFrame:
    ...
    if "scheduledDeparture" in df.columns:
        ...
        df["scheduledDeparture"] = df["scheduledDeparture"].dt.tz_convert(
            OSLO_TZ)
    ...
    if "isDelayed" in df.columns:
        df["isDelayed"] = pd.to_numeric(
            df["isDelayed"], errors="coerce").fillna(0).astype(int)
...
if df.empty:
    ...
    if time_range == "Siste 24 timer":
        df_time = df[df["scheduledDeparture"]
                     >= (now_oslo - timedelta(hours=24))]
    elif time_range == "Siste 7 dager":
        df_time = df[df["scheduledDeparture"]
                     >= (now_oslo - timedelta(days=7))]
    elif time_range == "Siste 30 dager":
        df_time = df[df["scheduledDeparture"]
                     >= (now_oslo - timedelta(days=30))]
    else:
        df_time = df

    destinations = sorted(df_time["destination"].dropna().unique(
    ).tolist()) if "destination" in df_time.columns else []
    selected_destinations = st.sidebar.multiselect(
        "Slutt-destinasjon", destinations, default=destinations)

    route_names = sorted(df_time["routeName"].fillna("NA").unique().tolist())
    selected_route_names = st.sidebar.multiselect(
        "Linje / rute-navn", route_names, default=route_names)

    transport_modes = sorted(
        df_time["transportMode"].fillna("NA").unique().tolist())
    selected_transport_modes = st.sidebar.multiselect(
        "Transporttype", transport_modes, default=transport_modes)

    only_delayed = st.sidebar.checkbox("Vis kun forsinkede", value=False)
    min_delay_min = st.sidebar.slider(
        "Min forsinkelse (min)",
        min_value=0,
        max_value=60,
        value=0,
    ...
    if selected_destinations:
        df_filtered = df_filtered[df_filtered["destination"].isin(
            selected_destinations)]

    if selected_route_names and "routeName" in df_filtered.columns:
        df_filtered = df_filtered[df_filtered["routeName"].fillna(
            "NA").isin(selected_route_names)]

    if selected_transport_modes:
        df_filtered = df_filtered[df_filtered["transportMode"].fillna(
            "NA").isin(selected_transport_modes)]
    ...
    if min_delay_min > 0 and "delaySeconds" in df_filtered.columns:
        df_filtered = df_filtered[df_filtered["delaySeconds"] >= (
            min_delay_min * 60)]
    ...
    with kpi_cols[1]:
        st.metric(
            "Forsinkede tog", int(
                df_filtered["isDelayed"].sum()) if "isDelayed" in df_filtered.columns else 0)
    with kpi_cols[2]:
        avg_delay = df_filtered["delaySeconds"].mean(
        ) / 60 if "delaySeconds" in df_filtered.columns else 0
    ...
    if not df_filtered.empty and "delaySeconds" in df_filtered.columns:
        chart_data = (
            df_filtered.groupby("destination")["delaySeconds"].mean() /
            60).sort_values(
            ascending=False)
    ...
    if df_route.empty:
        ...
        df_route = df_route.dropna(
            subset=["scheduledDeparture"]).set_index("scheduledDeparture")
        ...
        if metric == "Snittforsinkelse (min)":
            series = (
                df_route["delaySeconds"].resample(rule).mean() /
                60).to_frame(
                name="Snitt (min)")
        else:
            series = (
                df_route["isDelayed"].resample(rule).mean() *
                100).to_frame(
                name="Andel (%)")
    ...
    df_table = df_table[columns_for_table].sort_values(
